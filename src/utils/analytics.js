...
function createComparisonIndex(rows) {
  return rows.reduce((map, row) => {
    ...
    bucket.frontOnSaleSkcCount += parseNumeric(row?.frontOnSaleSkcCount);
    map[key] = bucket;
    return map;
  }, {});
}
...
export function buildComparisonMap(currentRows, previousRows, periodSpanDays = 0) {
  const metricKeys = ['totalSales', 'backstageNewCount', 'pricePassCount', 'templateCount', 'publishFrontCount', 'frontOnSaleSkcCount'];
