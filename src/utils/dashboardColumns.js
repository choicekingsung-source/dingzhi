...
const METRIC_DEFS = [
  ...
  { key: 'frontOnSaleSkcCount', title: '前端在售SKC数量' },
...
function formatCompareText(current, previous) {
  ...
  const rounded = `${rate >= 0 ? '+' : ''}${Math.round(rate * 100)}%`;
  if (rate > 0) return { text: rounded, color: '#ff4d4f' };
  if (rate < 0) return { text: rounded, color: '#52c41a' };
  return { text: '0%', color: '#8c8c8c' };
}

function renderProgressCell(label, currentValue, targetValue) {
...
export function createDashboardColumns({ rows, monthlyTargets, selectedMonth }) {
  ...
  const metricColumns = METRIC_DEFS.map((metric) => ({
    ...
    render: (value, row) => {
      ...
      if (!trend) {
        return <div className="metric-cell"><span>{currentValue}</span></div>;
      }

      return (
        <div className="metric-cell">
          <span>{currentValue}</span>
          <Tag color={trend.color === '#ff4d4f' ? 'error' : trend.color === '#52c41a' ? 'success' : 'default'}>
            环比 {trend.text}
          </Tag>
        </div>
      );
  ...
  return [
    ...
    ...metricColumns,
    {
      ...
      width: 240,
      render: (_, row) => {
        const key = buildTargetKey(row?.storeName, selectedMonth);
        const targetValue = monthlyTargets[key]?.backstageNewCount;
        return renderProgressCell(`后台上新数量（${selectedMonth || '未选择月份'}）`, row?.backstageNewCount, targetValue);
      },
    },
    {
      title: '销量目标进度条',
      ...
      render: (_, row) => {
        const key = buildTargetKey(row?.storeName, selectedMonth);
        const targetValue = monthlyTargets[key]?.totalSales;
        return renderProgressCell(`总销量（${selectedMonth || '未选择月份'}）`, row?.totalSales, targetValue);
      },
    },
  ];
}
