...
function mergeRows(existingRows, incomingRows) {
  ...
  [...existingRows, ...incomingRows].forEach((row, index) => {
    ...
    merged.push({
      ...row,
      __rowKey: row?.__rowKey || `merged-${index}`,
    });
  });
...
export function appendRows(existingRows, incomingRows) {
  return mergeRows(existingRows || [], incomingRows || []);
}
...
export async function appendRowsInCloud(existingRows, incomingRows) {
  const merged = mergeRows(existingRows, incomingRows);
